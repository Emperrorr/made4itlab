...
##    included = self.included_services.as_xml()
##    excluded = self.excluded_services.as_xml()
    
    def as_xml(self):
        result = [
            f'<Services>{self.included_services}</Services>',
            f'<ExcludedServices>{self.excluded_services}</ExcludedServices>'
            ]
        result = '\n  '.join(result)
